...
############################## Eliminación ##################################
    ...
    def dibujar_arbol(self, nodo, x, y, espaciado, ax):
        ...
        if nodo.izquierda:
            ...
            self.dibujar_arbol(nodo.izquierda, x - espaciado,
                               y - 1, espaciado / 2, ax)
        if nodo.derecha:
            ax.plot([x, x + espaciado], [y, y - 1], '-k')
            self.dibujar_arbol(nodo.derecha, x + espaciado,
                               y - 1, espaciado / 2, ax)
    ...
    def dibujar(self):
        fig, ax = plt.subplots()
        # Modificar el último parámetro para ajustar el espaciado horizontal
        self.dibujar_arbol(self.raiz, 0, 0, 4, ax)
        ax.set_aspect('equal')
        ax.axis('off')
